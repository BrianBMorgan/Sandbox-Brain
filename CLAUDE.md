...
1. **`scripts/refreshbrain.sh`** — batch sweep over `REPOS`. Needs `BRAIN_API_KEY` exported in the environment. Sends `embeddings:true` on every analyze (incremental — only new/changed symbols are embedded, so it's cheap on no-op runs). Honors an optional repo filter: `REPO_ONLY="A,B"` (run only those) / `REPO_SKIP="A,B"` (run all but those), names matching the repo basename; unset = all repos. The nightly cron is **split into two scheduled workflows** that share the `brain-refresh` concurrency group (so they never overlap):
   - `.github/workflows/refresh-cron.yml` — the **core sweep**, every repo EXCEPT `Sandbox-GTM` (`REPO_SKIP=Sandbox-GTM`), 08:00 UTC + manual `workflow_dispatch`.
   - `.github/workflows/refresh-gtm.yml` — **`Sandbox-GTM` only** (`REPO_ONLY=Sandbox-GTM`), 11:00 UTC + manual. GTM is the largest repo (~8.8k embeddings) and its embeddings phase OOM-kills the gitnexus worker on the current Render plan; it used to fail the whole nightly red even though the other six refreshed fine. Quarantining it keeps the core sweep green, makes GTM's red its own, and lets GTM run with the brain settled and to itself (its best shot). If GTM still OOMs solo, the next lever is trimming its index footprint (exclude generated/vendored files), **not** more RAM (and never `GITNEXUS_MAX_MEM_MB` — see the Memory invariant).
