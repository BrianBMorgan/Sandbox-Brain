...
- **Indexed repos** = the `REPOS` list in `scripts/refreshbrain.sh`: `Sandbox-Group-LLC/SYSOI.ai`, `Forge-Intelligence`, `Pitch-Box`, `Sandbox-GTM`, `Sandbox-ERP`, `ForgeOS`, `Content-Brain` (7). A per-run subset is selectable via `BRAIN_ONLY` / `BRAIN_SKIP` (see "Keeping the brain fresh"). This repo is **not** indexed into the brain — it's the host.
