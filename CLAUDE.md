...
- **Live consumers:** the SYSOI Brain console (`/app/brain`, BFF `POST /api/brain/chat`) and the **Slack bot** (`@brain` — SYSOI `src/modules/brain/slack.ts`; default = fleet-grounded strategist, one MCP `query` per roster repo per question). Both implement this exact handshake. Slack wiring, message grammar, and SYSOI env surface: **`docs/SLACK-INTEGRATION.md`**.
