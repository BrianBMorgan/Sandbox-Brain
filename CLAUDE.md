...
- **Indexed repos** = the `REPOS` list in `scripts/refreshbrain.sh` (all under `Sandbox-Group-LLC`): `SYSOI.ai`, `Forge-Intelligence`, `Pitch-Box`, `Sandbox-GTM`, `Sandbox-ERP`, `ForgeOS`, `Content-Brain` (7). Those basenames are the `reponame` values a per-run subset selects on via `BRAIN_ONLY` / `BRAIN_SKIP` (see "Keeping the brain fresh"). This repo is **not** indexed into the brain — it's the host.
