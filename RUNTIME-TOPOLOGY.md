...
- **`GITNEXUS_HOME` is load-bearing.** `getGlobalDir()` = `process.env.GITNEXUS_HOME || os.homedir()/.gitnexus` (`repo-manager.js:286`). The override supplies `HOME=/data`, so the persistent index at `/data/.gitnexus` is found. Clearing the override *without* setting `GITNEXUS_HOME` makes the entrypoint's `gosu node gitnexus serve` resolve `~/.gitnexus` → `/home/node/.gitnexus` (ephemeral) → **the brain boots with 0 repos.**
